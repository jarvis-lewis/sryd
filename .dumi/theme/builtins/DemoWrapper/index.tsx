...
import { BugOutlined, CodeOutlined, ExperimentOutlined } from "@sryd/icons";
  ...
  return (
    ...
    >
      <Global
        styles={css`
          :root {
            --antd-site-api-deprecated-display: ${showDebug
              ? "table-row"
              : "none"};
          }
        `}
      ...
      <span className="all-code-box-controls">
        ...
        <Tooltip
          title={
            <FormattedMessage
              id={`app.component.examples.${showDebug ? "hide" : "visible"}`}
            />
          }
        >
          <Button
        ...
        <Tooltip
          title={enableCssVar ? locale.disableCssVar : locale.enableCssVar}
        >
          <Button
            type="text"
            size="small"
