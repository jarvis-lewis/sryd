...
import { LinkOutlined, ThunderboltOutlined, UpOutlined } from "@sryd/icons";
...
const CodePreviewer: React.FC<AntdPreviewerProps> = (props) => {
  ...
  const mergedChildren =
    !iframe && clientOnly ? <ClientOnly>{children}</ClientOnly> : children;
  ...
  const dependencies = (jsx as string)
    .split("\n")
  ...
  dependencies["@sryd/icons"] = "latest";
  ...
  const riddlePrefillConfig = {
    title: `${localizedTitle} - antd@${dependencies.antd}`,
    js: `${
      /import React(\D*)from 'react';/.test(jsx)
        ? ""
...
createRoot(document.getElementById('container')).render(<Demo />);
  ...
  if (suffix === "tsx") {
    stackblitzPrefillConfig.files["tsconfig.json"] = JSON.stringify(
      tsconfig,
      null,
      2,
    );
      ...
      {!simplify && (
        <section className="code-box-meta markdown">
          <div className="code-box-title">
            <Tooltip
              title={
                originDebug ? <FormattedMessage id="app.demo.debug" /> : ""
              }
            >
  ...
  if (version) {
    return (
      <Badge.Ribbon
        text={version}
        color={version.includes("<") ? "red" : undefined}
      >
        {codeBox}
      </Badge.Ribbon>
    );
