...
import { presetDarkPalettes } from "@sryd/colors";
...
const Palette: React.FC<PaletteProps> = (props) => {
  ...
  useEffect(() => {
    ...
    Object.keys(colorNodesRef.current || {}).forEach((key) => {
      const computedColor = getComputedStyle(colorNodesRef.current[key])[
        "background-color"
      ];
  ...
  const [lastColor, firstColor] = dark
    ? colorPaletteMap.dark
    : colorPaletteMap.default;
