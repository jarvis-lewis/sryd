...
describe("InputNumber", () => {
  ...
  it("should call onStep when press up or down button", () => {
    ...
    const { container } = render(
      <InputNumber defaultValue={1} onStep={onStep} />,
    );
    fireEvent.mouseDown(
      container.querySelector(".ant-input-number-handler-up")!,
    );
    expect(onStep).toHaveBeenCalledTimes(1);
    expect(onStep).toHaveBeenLastCalledWith(2, { offset: 1, type: "up" });

    fireEvent.mouseDown(
      container.querySelector(".ant-input-number-handler-down")!,
    );
  ...
  it("should support className", () => {
    ...
    expect(
      container
        .querySelector(".anticon-arrow-up")
        ?.className.includes("my-class-name"),
    ).toBe(true);
    expect(
      container
        .querySelector(".anticon-arrow-down")
        ?.className.includes("my-class-name"),
    ).toBe(true);
  });
  ...
  it("Deprecation and usage tips", () => {
    ...
    const { container } = render(
      <InputNumber bordered={false} type="number" changeOnWheel />,
    );
    ...
    expect(
      container.querySelector(".ant-input-number-borderless"),
    ).toBeTruthy();
