...
import { createCache, extractStyle, StyleProvider } from "@sryd/cssinjs";
...
describe("message.hooks", () => {
  ...
  it("should work", () => {
              ...
              onClick={() => {
                api.open({
                  ...
                  content: (
                    <Context.Consumer>
                      {(name) => (
                        <span className="hook-test-result">{name}</span>
                      )}
                    </Context.Consumer>
                  ),
                });
    ...
    expect(document.querySelectorAll(".my-test-message-notice")).toHaveLength(
      1,
    );
    expect(document.querySelector(".hook-test-result")!.textContent).toEqual(
      "bamboo",
    );
  });
