...
import { LikeOutlined, SmileOutlined } from "@sryd/icons";
...
describe("Typography copy", () => {
  ...
  describe("Base", () => {
    describe("copyable", () => {
      function copyTest({
        ...
        it(name, async () => {
          ...
          if (iconTexts[0] !== undefined) {
            expect(
              container.querySelectorAll(".ant-typography-copy")[0].textContent,
            ).toBe(iconTexts[0]);
          }

          fireEvent.mouseEnter(
            container.querySelectorAll(".ant-typography-copy")[0],
          );
          await waitFakeTimer();
          ...
          if (tooltipLength !== undefined) {
            await waitFor(() => {
              expect(
                container.querySelectorAll(".ant-tooltip-inner").length,
              ).toBe(tooltipLength);
            });
          }
          ...
          if (tooltipTexts[1] !== undefined) {
            const expectedInner =
              tooltipTexts[1] === "" ? tooltipTexts[0] : tooltipTexts[1];
            await waitFor(() => {
              expect(
                container.querySelector(".ant-tooltip-inner")?.textContent,
              ).toBe(expectedInner);
      ...
      copyTest({
        name: "icon custom text",
        icon: ["a", "b"],
        iconTexts: ["a", "b"],
      });
      copyTest({
        name: "icon custom element",
        icon: [dom, dom2],
        iconTexts: ["12", "34"],
      });
      copyTest({
  ...
  it("tabIndex of copy button", () => {
    ...
    expect(
      container.querySelector(".ant-typography-copy")?.getAttribute("tabIndex"),
    ).toBe("-1");
  });
