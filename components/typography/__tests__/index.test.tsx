...
describe("Typography", () => {
  ...
  const mockGetBoundingClientRect = jest.spyOn(
    HTMLElement.prototype,
  ...
  describe("Base", () => {
    describe("copyable", () => {
      ...
      function copyTest(
        ...
        it(name, async () => {
          ...
          if (tooltips === undefined || tooltips === true) {
            expect(
              container.querySelector(".ant-tooltip-inner")?.textContent,
            ).toBe("Copy");
      ...
      copyTest(
        "tooltips contains two empty text",
        "bamboo",
        "bamboo",
        undefined,
        ["", ""],
      ...
      copyTest(
        "tooltips contains one empty text",
        "bamboo",
        "bamboo",
        undefined,
        ["", "you clicked!!"],
      ...
      copyTest(
        "tooltips contains one empty text 2",
        "bamboo",
        "bamboo",
        undefined,
        ["click here", ""],
    ...
    describe("editable", () => {
      ...
      function testStep(
        ...
        it(name, async () => {
          ...
          if (triggerType === undefined || triggerType.includes("icon")) {
            if (icon) {
              expect(
                wrapper.querySelectorAll(".anticon-highlight").length,
              ).toBeGreaterThan(0);
            } else {
              expect(
                wrapper.querySelectorAll(".anticon-edit").length,
              ).toBeGreaterThan(0);
            }
            ...
            fireEvent.mouseEnter(
              wrapper.querySelectorAll(".ant-typography-edit")[0],
            );
            act(() => {
              jest.runAllTimers();
            });
            ...
            if (tooltip === undefined || tooltip === true) {
              await waitFor(() => {
                expect(
                  wrapper.querySelector(".ant-tooltip-inner")?.textContent,
                ).toBe("Edit");
              });
            } else if (tooltip === false) {
              await waitFor(() => {
                expect(
                  wrapper.querySelectorAll(".ant-tooltip-inner").length,
                ).toBe(0);
              });
            } else {
              await waitFor(() => {
            ...
            if (triggerType?.includes("text")) {
              fireEvent.keyDown(wrapper.querySelector("textarea")!, {
                keyCode: KeyCode.ESC,
              });
          ...
          if (enterIcon === undefined) {
            expect(
              wrapper.querySelectorAll(
                "span.ant-typography-edit-content-confirm",
              )[0].className,
      ...
      testStep({
        name: "customize edit tooltip text",
      ...
      testStep({
        name: "trigger by both icon and text",
        triggerType: ["icon", "text"],
      });
      ...
      it("should trigger onEnd when type Enter", () => {
        const onEnd = jest.fn();
        const { container: wrapper } = render(
          <Paragraph editable={{ onEnd }}>Bamboo</Paragraph>,
        );
      ...
      it("should trigger onStart when type Start", () => {
        const onStart = jest.fn();
        const { container: wrapper } = render(
          <Paragraph editable={{ onStart }}>Bamboo</Paragraph>,
        );
        fireEvent.click(wrapper.querySelectorAll(".ant-typography-edit")[0]);
        fireEvent.keyDown(wrapper.querySelector("textarea")!, {
          keyCode: KeyCode.A,
      ...
      it("should only trigger focus on the first time", () => {
        let triggerTimes = 0;
        const { container: wrapper } = render(
          <Paragraph editable>Bamboo</Paragraph>,
        );
    ...
    it("should focus at the end of textarea", () => {
      const { container: wrapper } = render(
        <Paragraph editable>content</Paragraph>,
      );
      fireEvent.click(wrapper.querySelectorAll(".ant-typography-edit")[0]);
