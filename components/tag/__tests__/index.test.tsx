...
import { CheckCircleOutlined } from "@sryd/icons";
...
describe("Tag", () => {
  ...
  it("should be closable", () => {
    ...
    expect(
      container.querySelectorAll(".ant-tag:not(.ant-tag-hidden)").length,
    ).toBe(1);
  ...
  it("show close button by closeIcon", () => {
    ...
    ["tag1", "tag2", "tag3", "tag4", "tag9", "tag10"].forEach((tag) => {
      expect(
        container.querySelector(`.${tag} .ant-tag-close-icon`),
      ).toBeTruthy();
    });
  ...
  it("should trigger onClick on Tag.CheckableTag", () => {
    const onClick = jest.fn();
    const { container } = render(
      <Tag.CheckableTag checked={false} onClick={onClick} />,
    );
    const tagElement = container.querySelector<HTMLSpanElement>(".ant-tag")!;
    fireEvent.click(tagElement);
    expect(onClick).toHaveBeenCalled();
  ...
  it("deprecated warning", () => {
    ...
    expect(errSpy).toHaveBeenCalledWith(
      "Warning: [sryd: Tag] `visible` is deprecated. Please use `visible && <Tag />` instead.",
    );
  ...
  it("should support aria-* in closable", () => {
    const { container } = render(
      <Tag closable={{ closeIcon: "X", "aria-label": "CloseBtn" }} />,
    );
    expect(
      container
        .querySelector(".ant-tag-close-icon")
        ?.getAttribute("aria-label"),
    ).toEqual("CloseBtn");
    expect(container.querySelector(".ant-tag-close-icon")?.textContent).toEqual(
      "X",
    );
  });
});
