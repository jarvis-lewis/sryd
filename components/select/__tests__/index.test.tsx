...
import { CloseOutlined } from "@sryd/icons";
...
describe("Select", () => {
  ...
  it("should have default notFoundContent", () => {
    ...
    expect(container.querySelectorAll(".ant-select-item-option").length).toBe(
      0,
    );
  ...
  it("should not have notFoundContent when mode is combobox and notFoundContent is set", () => {
    ...
    expect(container.querySelector(".ant-select-item-empty")).toHaveTextContent(
      "not at all",
    );
  });
  ...
  describe("clear icon position", () => {
    it("normal", () => {
      ...
      expect(
        getComputedStyle(container.querySelector(".ant-select-clear")!)
          .insetInlineEnd,
      ).toEqual("11px");
    });
    ...
    it("deprecate bordered", () => {
      ...
      expect(errSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          "Warning: [antd: Select] `bordered` is deprecated",
        ),
      );
