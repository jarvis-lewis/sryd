...
// ============================== Group ==============================
const floatButtonGroupStyle: GenerateStyle<FloatButtonToken, CSSObject> = (
  token,
) => {
  const {
    antCls,
  ...
  return {
    ...
    [`${groupPrefixCls}-square`]: {
      [`${componentCls}-square`]: {
        ...
        [`${antCls}-badge`]: {
          [`${antCls}-badge-count`]: {
            top: calc(calc(floatButtonBodyPadding).add(badgeOffset))
              .mul(-1)
              .equal(),
            insetInlineEnd: calc(calc(floatButtonBodyPadding).add(badgeOffset))
              .mul(-1)
              .equal(),
          },
        },
...
// ============================== Shared ==============================
const sharedFloatButtonStyle: GenerateStyle<FloatButtonToken, CSSObject> = (
  token,
) => {
  const {
    antCls,
...
export default genStyleHooks(
  ...
  (token) => {
    ...
    const floatButtonToken = mergeToken<FloatButtonToken>(token, {
      ...
      floatButtonBodySize: calc(controlHeightLG)
        .sub(calc(paddingXXS).mul(2))
        .equal(),
