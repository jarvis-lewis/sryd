...
describe("Button", () => {
  ...
  it("renders Chinese characters correctly", () => {
    expect(
      render(<Button>按钮</Button>).container.firstChild,
    ).toMatchSnapshot();
    // should not insert space when there is icon
    expect(
      render(<Button icon={<SearchOutlined />}>按钮</Button>).container
        .firstChild,
    ).toMatchSnapshot();
    // should not insert space when there is icon
    expect(
    ...
    expect(
      render(<Button loading>按钮</Button>).container.firstChild,
    ).toMatchSnapshot();
  ...
  it("should change loading state instantly by default", () => {
    ...
    expect(wrapper.container.querySelectorAll(".ant-btn-loading").length).toBe(
      1,
    );
  });
  ...
  it("fixbug renders {0} , 0 and {false}", () => {
    ...
    expect(render(<Button>0</Button>).container.firstChild).toMatchSnapshot();
    expect(
      render(<Button>{false}</Button>).container.firstChild,
    ).toMatchSnapshot();
  });
  ...
  it("should match class .ant-btn-disabled when button is disabled and href is not undefined", () => {
    ...
    expect(wrapper.container.querySelector(".ant-btn")).toHaveClass(
      "ant-btn-disabled",
    );
  });
  ...
  it("should prevent children's event when button is disabled", () => {
    ...
    expect(
      window.getComputedStyle(container.querySelector("#link")!).pointerEvents,
    ).toBe("none");
  });
  ...
  it("should display loading when delay is zero", () => {
    const { container } = render(
      <Button loading={{ delay: 0 }}>Button</Button>,
    );
    expect(container.querySelectorAll(".ant-btn-loading").length).toBe(1);
  });
  ...
  it("should support autoInsertSpace", () => {
    const text = "确定";
    const { container } = render(
      <Button autoInsertSpace={false}>{text}</Button>,
    );
    expect(
      container.querySelector<HTMLButtonElement>("button")?.textContent,
    ).toBe(text);
  });
  ...
  it("should support solidTextColor when theme changes", () => {
    const { container: defaultContainer } = render(
      <ConfigProvider
        theme={{ algorithm: [theme.defaultAlgorithm], cssVar: true }}
      >
