...
const genTransferListStyle: GenerateStyle<TransferToken> = (
  ...
  const contentBorderRadius = unit(
    token.calc(borderRadiusLG).sub(lineWidth).equal(),
  );
...
export const prepareComponentToken: GetDefaultToken<"Transfer"> = (token) => {
  const { fontSize, lineHeight, controlHeight, controlHeightLG, lineWidth } =
    token;
  ...
  return {
    ...
    transferHeaderVerticalPadding: Math.ceil(
      (controlHeightLG - lineWidth - fontHeight) / 2,
    ),
  };
};
