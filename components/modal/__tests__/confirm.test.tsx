...
describe("Modal.confirm triggers callbacks correctly", () => {
  ...
  it("allows extra props on buttons", async () => {
    await open({
      ...
      cancelButtonProps: {
        "data-test": "baz",
      } as ModalFuncProps["cancelButtonProps"],
    });

    expect($$(".ant-btn")).toHaveLength(2);
    expect(($$(".ant-btn")[0].attributes as any)["data-test"].value).toBe(
      "baz",
    );
  ...
  describe("could be update by new config", () => {
    (["info", "success", "warning", "error"] as const).forEach((type) => {
      it(type, async () => {
        ...
        expect($$(".ant-modal-confirm-content")[0].innerHTML).toBe(
          "new content",
        );
        instance.destroy();
        await waitFakeTimer();
  ...
  describe("could be update by call function", () => {
    (["info", "success", "warning", "error"] as const).forEach((type) => {
      it(type, async () => {
        ...
        expect($$(".ant-modal-confirm-title")[0].innerHTML).toBe("title");
        expect(
          $$(".ant-modal-confirm-btns .ant-btn-primary")[0].classList,
        ).toContain("ant-btn-loading");
        expect(
          $$(".ant-modal-confirm-btns .ant-btn-primary")[0].style.color,
        ).toBe("red");
  ...
  it("icon can be null to hide icon", async () => {
    ...
    expect(
      document.querySelector(".ant-modal-confirm-body")!.children,
    ).toHaveLength(1);
    expect(
      document
  ...
  it("should be able to config holderRender", async () => {
    ...
    expect(
      document.querySelectorAll(".anticon-exclamation-circle"),
    ).toHaveLength(0);
    expect(document.querySelectorAll(".test-modal-root")).toHaveLength(1);
    expect(document.querySelectorAll(".icon-exclamation-circle")).toHaveLength(
      1,
    );
  ...
  it("should be able to config holderRender and static config", async () => {
    ...
    ConfigProvider.config({
      prefixCls: "prefix-1",
      holderRender: (children) => (
        <ConfigProvider prefixCls="prefix-2">{children}</ConfigProvider>
      ),
    });
    Modal.confirm({ content: "hai" });
    await waitFakeTimer();
  ...
  it("should be able to config holderRender antd locale", async () => {
    ...
    ConfigProvider.config({
      holderRender: (children) => (
        <ConfigProvider locale={{ Modal: { okText: "test" } } as any}>
          {children}
        </ConfigProvider>
      ),
    });
    Modal.confirm({ content: "hai" });
    await waitFakeTimer();
    expect(document.querySelector(".ant-btn-primary")?.textContent).toBe(
      "test",
    );
    ConfigProvider.config({ holderRender: undefined });
  });
