...
describe("notification", () => {
  ...
  it("should be able to hide manually", async () => {
    ...
    expect(document.querySelectorAll(".ant-notification-notice")).toHaveLength(
      2,
    );
  ...
  it("should be able to global config rootPrefixCls", async () => {
    ...
    expect(
      document.querySelectorAll(".prefix-test-notification-notice"),
    ).toHaveLength(1);
  ...
  it("should be able to open with icon", async () => {
    ...
    list.forEach((type) => {
      expect(document.querySelectorAll(`${iconPrefix}-${type}`)).toHaveLength(
        1,
      );
    });
  });
  ...
  it("should be able to add parent class for different notification types", async () => {
    ...
    list.forEach((type) => {
      expect(
        document.querySelectorAll(`.ant-notification-notice-${type}`),
      ).toHaveLength(1);
    });
  });
  ...
  it("closeIcon should be update", async () => {
    ...
    list.forEach((type) => {
      expect(
        document.querySelector(`.test-customize-icon-${type}`),
      ).toBeTruthy();
    });
  });
  ...
  it("support props", () => {
    ...
    expect(
      document.querySelectorAll("[data-testid='test-notification']").length,
    ).toBe(1);
  });
  ...
  it("should hide close btn when closeIcon setting to null or false", async () => {
    ...
    await awaitPromise();
    expect(
      document.querySelectorAll(".normal .ant-notification-notice-close")
        .length,
    ).toBe(1);
    expect(document.querySelectorAll(".custom .custom-close-icon").length).toBe(
      1,
    );
    expect(
      document.querySelectorAll(".with-null .ant-notification-notice-close")
        .length,
    ).toBe(0);
    expect(
      document.querySelectorAll(".with-false .ant-notification-notice-close")
        .length,
    ).toBe(0);
  });
  ...
  it("style.width could be override", async () => {
    ...
    await awaitPromise();
    expect(document.querySelector(".with-style")).toHaveStyle({
      width: "600px",
    });
  });
});
