...
import { AlertFilled } from "@sryd/icons";
import type { ColProps } from "sryd/es/grid";
...
describe("Form", () => {
  ...
  describe("noStyle Form.Item", () => {
    it("should show alert when form field is required but empty", async () => {
      ...
      ).toHaveTextContent("'test' is required");
  ...
  it("render functions require either `shouldUpdate` or `dependencies`", () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      "Warning: [sryd: Form.Item] A `Form.Item` with a render function must have either `shouldUpdate` or `dependencies`.",
    );
  });
  ...
  it("`shouldUpdate` shouldn't work with `dependencies`", () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      "Warning: [sryd: Form.Item] `shouldUpdate` and `dependencies` shouldn't be used together. See https://u.ant.design/form-deps.",
    );
  });
  ...
  it("`name` should not work with render props", () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      "Warning: [sryd: Form.Item] A `Form.Item` with a render function cannot be a field, and thus cannot have a `name` prop.",
    );
  });
  ...
  it("multiple children with a name prop", () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      "Warning: [sryd: Form.Item] A `Form.Item` with a `name` prop must have a single child element. For information on how to render more complex form items, see https://u.ant.design/complex-form-item.",
    );
  });
  ...
  it("input element should have the prop aria-describedby pointing to the help id when there is a help message", () => {
    ...
    expect(container.querySelector(".ant-form-item-explain")?.id).toBe(
      "test_help",
    );
  });
  ...
  it("input element should have the prop aria-required when the prop `required` is true", () => {
    ...
    expect(
      container.querySelector("input")?.getAttribute("aria-required"),
    ).toBe("true");
  });
  ...
  it("input element should have the prop aria-describedby pointing to the extra id when there is a extra message", () => {
    ...
    expect(container.querySelector(".ant-form-item-extra")?.id).toBe(
      "test_extra",
    );
  });
  ...
  describe("scrollToFirstError", () => {
    ...
    it("should work with scrollToFirstError with ref", async () => {
      const ForwardRefInput = React.forwardRef<HTMLInputElement, any>(
        ({ id, ...props }, ref) => <input {...props} ref={ref} />,
      );

      const NativeInput = React.forwardRef<any, any>(
        ({ id, ...props }, ref) => {
    ...
    it("should work with Upload", async () => {
      ...
      expect((scrollIntoView as any).mock.calls[0][0]).toBe(
    ...
    it("should not throw error when use InputNumber", async () => {
      ...
      expect(scrollIntoView).toHaveBeenCalled();
      expect((scrollIntoView as any).mock.calls[0][0]).toBe(
        inputNumberRef.current?.nativeElement,
      );
    });
  });
  ...
  it("warning when use `name` but children is not validate element", () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      "Warning: [sryd: Form.Item] `name` is only used for validate React element. If you are using Form.Item as layout display, please remove `name` instead.",
    );
  });
  ...
  it("dynamic change required", async () => {
    const { container } = render(
      <Form>
        ...
        <Form.Item
          ...
          dependencies={["light"]}
          rules={[
            ({ getFieldValue }) => ({ required: getFieldValue("light") }),
          ]}
        >
          <input />
        </Form.Item>
    ...
    expect(
      container.querySelector(".ant-form-item-explain-error"),
    ).toHaveTextContent("'bamboo' is required");
  });
  ...
  describe("should show related className when customize help", () => {
    it("normal", async () => {
      ...
      expect(
        container.querySelector(".ant-form-item-explain"),
  ...
  it("warning when use v3 function", () => {
    Form.create();
    expect(errorSpy).toHaveBeenCalledWith(
      "Warning: [sryd: Form] sryd v4 removed `Form.create`. Please remove or use `@ant-design/compatible` instead.",
    );
  });
  ...
  it("warning when use `dependencies` but `name` is empty & children is not a render props", () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      "Warning: [sryd: Form.Item] Must set `name` or use a render function when `dependencies` is set.",
    );
  });
  ...
  it("not repeat render when Form.Item is not a real Field", async () => {
    ...
    const Demo = () => {
      ...
      return (
        <Form form={form}>
          ...
          <Button
            id="fill-btn"
            onClick={() => form.setFieldValue("light", "bamboo")}
          >
            fill
          </Button>
        </Form>
    ...
    expect(
      container.querySelector<HTMLInputElement>("#changed")!.value,
    ).toEqual("");
  ...
  it("empty help should also render", () => {
    ...
    expect(
      container.querySelectorAll(".ant-form-item-explain").length,
    ).toBeTruthy();
  });
  ...
  it("Form.Item with `help` should display error style when validate failed", async () => {
    ...
    await changeValue(0, "");
    expect(container.querySelector(".ant-form-item")).toHaveClass(
  ...
  it("`null` triggers warning and is treated as `undefined`", () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      "Warning: [sryd: Form.Item] `null` is passed as `name` property",
    );
  });
  ...
  it("should not throw error when Component.props.onChange is null", async () => {
    const CustomComponent: React.FC = () => (
      <input
        onChange={null as unknown as ChangeEventHandler<HTMLInputElement>}
      />
    );
    render(
      <Form>
  ...
  it("`label` support template", async () => {
    ...
    expect(
      container.querySelector(".ant-form-item-explain-error"),
    ).toHaveTextContent("Bamboo is good!");
  });
  ...
  it("validation message should has alert role", async () => {
    ...
    expect(
      container.querySelector(".ant-form-item-explain-error"),
    ).toHaveTextContent("name is good!");
  });
  ...
  it("should avoid re-render", async () => {
    ...
    const MyInput: React.FC<{ value?: string }> = ({
      value = "",
      ...props
    }) => {
      renderTimes += 1;
  ...
  it("should warning with `defaultValue`", () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      "Warning: [sryd: Form.Item] `defaultValue` will not work on controlled Field. You should use `initialValues` of Form instead.",
    );
  });
  ...
  it("form.item should support label = null", () => {
    ...
    const oneItems = items[0]
      .querySelector(".ant-row")
      ?.querySelectorAll(".ant-col");
  ...
  describe("tooltip", () => {
    it("ReactNode", async () => {
      ...
      expect(container.querySelector(".ant-tooltip-inner")).toHaveTextContent(
        "Bamboo",
      );
    });
  ...
  describe("form colon", () => {
    it("default colon", () => {
      ...
      expect(screen.getByText("姓名")).not.toHaveClass(
  ...
  describe("noStyle with status", () => {
    it("noStyle should not affect status", async () => {
      ...
      expect(container.querySelector(".custom-select")).toHaveClass(
        "ant-select-status-error",
      );
      expect(container.querySelector(".custom-select")).not.toHaveClass(
        "ant-select-in-form-item",
      );

      expect(container.querySelector(".custom-select-b")).toHaveClass(
        "ant-select-status-error",
      );
      expect(container.querySelector(".custom-select-b")).toHaveClass(
        "ant-select-in-form-item",
      ...
      expect(container.querySelector(".custom-select-c")).toHaveClass(
      ...
      expect(container.querySelector(".custom-select-d")).toHaveClass(
        "ant-select-status-warning",
      );
      expect(container.querySelector(".custom-select-d")).toHaveClass(
        "ant-select-in-form-item",
      ...
      expect(container.querySelector(".custom-select-e")).toHaveClass(
        "ant-select-in-form-item",
      );
    });
    ...
    it("parent pass status", async () => {
      ...
      ).toEqual("'first' is required");

      expect(container.querySelectorAll("input")[0]).toHaveClass(
        "ant-input-status-error",
      );
      expect(container.querySelectorAll("input")[1]).not.toHaveClass(
        "ant-input-status-error",
      );
    });
  });
  ...
  it("should not affect Popup children style", () => {
    ...
    expect(container.querySelector(".modal-select")?.className).not.toContain(
      "in-form-item",
    );
    expect(container.querySelector(".modal-select")?.className).not.toContain(
      "status-error",
    );
    expect(container.querySelector(".drawer-select")?.className).not.toContain(
      "in-form-item",
    );
    expect(container.querySelector(".drawer-select")?.className).not.toContain(
      "status-error",
  ...
  it("Form.Item.useStatus should work", async () => {
    ...
    const CustomInput: React.FC<{
      ...
      return (
        <div className={classNames(className, `custom-input-status-${status}`)}>
          {value}
        </div>
      );
    };
    ...
    const Demo: React.FC = () => {
      ...
      return (
        <Form form={form} name="test-form">
          <Form.Item
            name="error"
            rules={[{ required: true, message: "This is a error message." }]}
          >
            <ErrorItem />
          </Form.Item>
          <Form.Item
  ...
  it("item customize margin", async () => {
    const computeSpy = jest
      ...
      .mockImplementation(
        () => ({ marginBottom: 24 }) as unknown as CSSStyleDeclaration,
      );
  ...
  it("form child components should be given priority to own disabled props when it in a disabled form", () => {
    ...
    const renderComps = (disabled?: boolean) => [
      ...
      <DatePicker.MonthPicker
        key="DatePicker.MonthPicker"
        disabled={disabled}
      />,
      <DatePicker.QuarterPicker
        key="DatePicker.QuarterPicker"
        disabled={disabled}
      />,
  ...
  it("success feedback should display when pass hasFeedback prop and current value is valid value", async () => {
    ...
    expect(
      container.querySelectorAll(".ant-form-item-has-feedback").length,
    ).toBe(0);
    expect(
      container.querySelectorAll(".ant-form-item-has-success").length,
    ).toBe(0);
  ...
  it("custom feedback icons should display when pass hasFeedback prop", async () => {
    ...
    expect(
      container.querySelectorAll("#custom-error-icon, #custom-error-icon2")
        .length,
    ).toBe(2);
  });
  ...
  it("validate status should be change in order", async () => {
    ...
    const initTriggerTime =
      ReactVersion.startsWith("18") || ReactVersion.startsWith("19") ? 2 : 1;
  ...
  describe("requiredMark", () => {
    ...
    it("should not render `requiredMark` when Form.Item has no required prop", () => {
      ...
      expect(
        container.querySelectorAll(".ant-form-item-required"),
      ).toHaveLength(2);
      expect(
        container.querySelectorAll(".ant-form-item-required-mark-optional"),
      ).toHaveLength(2);
    });
    ...
    it("customize logic", () => {
      ...
      expect(
        container.querySelectorAll(".ant-form-item-label")[0].textContent,
      ).toEqual("Required: true");
      expect(
        container.querySelectorAll(".ant-form-item-label")[1].textContent,
      ).toEqual("Optional: false");
    });
  });
  ...
  it("not warning for react key", async () => {
    ...
    function expectErrors(errors: string[]) {
      expect(
        container.querySelectorAll(".ant-form-item-explain-error"),
      ).toHaveLength(errors.length);
      errors.forEach((error, index) => {
        expect(
          container.querySelectorAll(".ant-form-item-explain-error")[index],
