      ...
      `]: {
        [itemCls]: {
          ...
          [`${itemCls}-thumbnail`]: {
            ...
            lineHeight: unit(
              calc(uploadThumbnailSize).add(token.paddingSM).equal(),
            ),
            textAlign: "center",
            flex: "none",
          ...
          [`${itemCls}-progress`]: {
            ...
            marginTop: 0,
            paddingInlineStart: calc(uploadThumbnailSize)
              .add(token.paddingXS)
              .equal(),
          },
        },
...
const genPictureCardStyle: GenerateStyle<UploadToken> = (token) => {
  const { componentCls, iconCls, fontSizeLG, colorTextLightSolid, calc } =
    token;
