...
describe("Menu", () => {
  ...
  it("If has select nested submenu item ,the menu items on the grandfather level should be highlight", () => {
    ...
    expect(
      container.querySelectorAll("li.ant-menu-submenu-selected").length,
    ).toBe(1);
  });
  ...
  it("should accept defaultOpenKeys in mode horizontal", () => {
    ...
    expect(
      container
        .querySelector(".ant-menu-submenu-open")
        ?.querySelector(".ant-menu-submenu-title")?.textContent,
    ).toEqual("submenu1");
  });
  ...
  it("should accept openKeys in mode horizontal", () => {
    ...
    triggerAllTimer();
    expect(
      container.querySelector("div.ant-menu-submenu-popup"),
    ).not.toHaveClass("ant-menu-submenu-hidden");
  });
  ...
  it("should accept openKeys in mode inline", () => {
    ...
    expect(container.querySelector("ul.ant-menu-sub")).not.toHaveClass(
      "ant-menu-hidden",
    );
  });
  ...
  describe("allows the overriding of theme at the popup submenu level", () => {
    const menuModesWithPopupSubMenu: MenuProps["mode"][] = [
      "horizontal",
      "vertical",
    ];
    menuModesWithPopupSubMenu.forEach((menuMode) => {
      it(`when menu is mode ${menuMode}`, () => {
        ...
        expect(container.querySelector("ul.ant-menu-root")).toHaveClass(
          "ant-menu-dark",
        );
        expect(
          container.querySelector("div.ant-menu-submenu-popup"),
        ).toHaveClass("ant-menu-light");
      });
    });
  });
  ...
  it("should always follow openKeys when inlineCollapsed is switched", () => {
    ...
    expect(container.querySelector("ul.ant-menu-sub")).toHaveClass(
      "ant-menu-inline",
    );
    expect(container.querySelector("li.ant-menu-submenu-inline")).toHaveClass(
      "ant-menu-submenu-open",
  ...
  it("inlineCollapsed should works well when specify a not existed default openKeys", () => {
    ...
    expect(container.querySelector(".ant-menu-submenu")).toHaveClass(
  ...
  it("inlineCollapsed Menu.Item Tooltip can be removed", () => {
    ...
    expect(
      container.querySelectorAll(".ant-tooltip-inner")[0].textContent,
    ).toBe("item");
    expect(
      container.querySelectorAll(".ant-tooltip-inner")[1].textContent,
    ).toBe("title");
    expect(
      container.querySelectorAll(".ant-tooltip-inner")[2].textContent,
    ).toBe("item");
  });
  ...
  describe("open submenu when click submenu title", () => {
    const toggleMenu = (
      ...
      fireEvent[event](
        instance.container.querySelectorAll(".ant-menu-submenu-title")[index],
      );
      triggerAllTimer();
    };
    ...
    it("vertical with click", () => {
      const defaultTestProps: MenuProps = {
        mode: "vertical",
  ...
  it("render correctly when using with Layout.Sider", () => {
    ...
    expect(container.querySelector("ul.ant-menu-root")).not.toHaveClass(
      "ant-menu-inline",
    );
    expect(container.querySelector("ul.ant-menu-root")).toHaveClass(
      "ant-menu-vertical",
  ...
  it("MenuItem should render icon and icon should be the first child when icon exists", () => {
    ...
    expect(container.querySelector(".ant-menu-item .anticon")).toHaveClass(
      "anticon-mail",
    );
  });
  ...
  it("should keep selectedKeys in state when collapsed to 0px", () => {
    ...
    const { container, rerender } = render(<Demo />);
    expect(
      container.querySelector("li.ant-menu-item-selected")?.textContent,
    ).toBe("Option 1");
  ...
  it("Use first char as Icon when collapsed", () => {
    ...
    expect(
      container.querySelectorAll(".ant-menu-inline-collapsed-noicon")[0]
        ?.textContent,
    ).toEqual("L");
    expect(
      container.querySelectorAll(".ant-menu-inline-collapsed-noicon")[1]
        ?.textContent,
    ).toEqual("B");
  });
  ...
  it("divider should show", () => {
    ...
    expect(container.querySelectorAll("li.ant-menu-item-divider").length).toBe(
      2,
    );
    expect(
      container.querySelectorAll("li.ant-menu-item-divider-dashed").length,
    ).toBe(1);
  });
  ...
  it("should not show icon when inlineCollapsed", () => {
    ...
    expect(container.querySelector(".bamboo")).toBeTruthy();
    expect(
      getComputedStyle(container.querySelector(".bamboo") as HTMLElement),
  ...
  it("Overflow indicator className should not override menu class", () => {
    ...
    expect(
      container.querySelector(".ant-menu.ant-menu-light.custom-popover"),
    ).toBeTruthy();
  });
  ...
  it("menu item with extra prop", () => {
    const text = "⌘P";
    const { container } = render(
      <Menu items={[{ label: "profile", key: "1", extra: text }]} />,
    );

    expect(
      container.querySelector(".ant-menu-title-content-with-extra"),
    ).toBeInTheDocument();
    expect(container.querySelector(".ant-menu-item-extra")?.textContent).toBe(
      text,
    );
  });
});
