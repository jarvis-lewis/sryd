...
import ConfigProvider from "sryd/es/config-provider";
import type { NotificationConfig } from "sryd/es/notification/interface";
...
describe("App", () => {
  ...
  it("should be a merged config configured in nested app", async () => {
    ...
    expect(offsetConsumedConfig?.message).toStrictEqual({
      maxCount: 1,
      top: 32,
    });
    expect(offsetConsumedConfig?.notification).toStrictEqual({
      maxCount: 2,
      top: 96,
    });
  ...
  it("should respect config from props in priority", async () => {
    ...
    const Wrapper = () => (
      <App
        message={{ maxCount: 10, top: 20 }}
        notification={{ maxCount: 30, bottom: 40 }}
      >
  ...
  it("should respect notification placement config from props in priority", async () => {
    ...
    expect(
      document.querySelector(".ant-notification-topRight"),
  ...
  it("support className", () => {
    ...
    expect(container.querySelector<HTMLDivElement>(".ant-app")).toHaveClass(
      "test-class",
    );
  });
  ...
  describe("component", () => {
    ...
    it("should warn if component is false and cssVarCls is not empty", () => {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        "Warning: [sryd: App] When using cssVar, ensure `component` is assigned a valid React component string.",
      );
    });
  });
});
